...
if (! class_exists('PWAFORWP_Plugin_Usage_Tracker') ) {
    include_once PWAFORWP_PLUGIN_DIR. '/admin/class-pwaforwp-plugin-usage-tracker.php';
}
...
if (pwaforwp_is_admin() ) {
    ...
    include_once PWAFORWP_PLUGIN_DIR."admin/settings.php";
...
function pwaforwp_init_plugin()
{
    ...
    include_once PWAFORWP_PLUGIN_DIR."/service-work/class-pwaforwp-service-worker.php"; 

    if (class_exists('WP_Service_Workers') ) { 
        include_once PWAFORWP_PLUGIN_DIR."/3rd-party/class-pwaforwp-wpwa.php"; 
    }
    //For CDN CODES
    if (! is_admin() ) {
...
function pwaforwp_admin_notice()
{
    ...
    if(( in_array($current_date, $list_of_date) 
        ...
        $pwaforwp_globe_admin_notice = true;
        echo sprintf(
            '<div class="updated notice is-dismissible message notice notice-alt pwaforwp-feedback-notice">
                    <p>
            ...
            esc_html__(
                'Excellent! You\'ve been using PWA For WP plugin for over a week. Hope you are enjoying it so far. We have spent countless hours developing this 
